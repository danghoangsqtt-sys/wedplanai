...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'fengshui', label: 'Phong Thủy', icon: Compass, roles: ['ADMIN', 'USER'] },
    ...
    { id: 'ai', label: 'Cố Vấn AI', icon: MessageSquareText, roles: ['ADMIN', 'USER'] },
  ...
  return (
    <>
      ...
      <aside 
        className={`
          fixed top-0 left-0 h-full bg-white border-r border-rose-100 shadow-2xl md:shadow-none 
          transition-transform duration-300 ease-out z-[100] md:z-auto md:relative flex flex-col
      ...
      >
        ...
        <div className={`h-16 flex items-center border-b border-rose-100 ${isCollapsed ? 'justify-center' : 'justify-between pl-6 pr-3'}`}>
           ...
           {!isCollapsed ? (
             ...
             <button 
               ...
               className="hidden md:flex p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
               title="Mở rộng"
             >
           ...
           {!isCollapsed && (
             <button 
               onClick={() => setIsCollapsed(!isCollapsed)}
               className="hidden md:flex p-1.5 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
               title="Thu gọn"
             >
           ...
           {/* Mobile Close Button */}
           <button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden p-2 text-gray-500 hover:bg-gray-100 rounded-lg">
             <X className="w-6 h-6" />
           </button>
        </div>
        ...
        {/* User Info (Mini) */}
        {user && (
          <div className={`px-6 py-4 border-b border-rose-50 bg-rose-50/30 flex items-center gap-3 ${isCollapsed ? 'hidden md:flex md:justify-center md:px-2' : ''}`}>
        ...
        <nav className="flex-1 py-4 flex flex-col gap-1 px-3 overflow-y-auto overscroll-contain">
          {filteredItems.map((item) => (
            <button
              ...
              className={`
                flex items-center gap-3 px-3 py-3.5 md:py-3 rounded-xl transition-all touch-manipulation
                ${activeTab === item.id 
            ...
            >
              ...
              {(!isCollapsed || window.innerWidth < 768) && <span className="text-sm">{item.label}</span>}
            </button>
          ))}
        </nav>
        ...
        {user?.role !== 'GUEST' && (
          <div className="p-4 border-t border-rose-100 bg-rose-50/30 space-y-2 pb-safe-area">
             <button 
                onClick={logout}
                className={`flex items-center gap-2 w-full text-gray-500 hover:text-red-500 hover:bg-red-50 p-2.5 rounded-lg transition-colors ${isCollapsed ? 'md:justify-center' : 'px-4'}`}
                title="Đăng xuất"
             >
                <LogOut className="w-5 h-5 flex-shrink-0" />
                {(!isCollapsed || window.innerWidth < 768) && <span className="text-sm font-medium">Đăng xuất</span>}
             </button>
          </div>
        )}
      </aside>
