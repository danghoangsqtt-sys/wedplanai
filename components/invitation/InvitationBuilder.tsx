                ...
                <div className={`flex-1 bg-gray-100 p-4 md:p-8 overflow-y-auto flex flex-col items-center justify-start lg:justify-center min-h-[700px] ${mobileView === 'EDIT' ? 'hidden lg:flex' : 'flex'}`}>
                    ...
                    >
                        ...
                        >
                            ...
                            {/* 2. CONTENT AREA (Nổi lên trên ảnh một chút) */}
                            <div className="flex-1 relative z-10 -mt-10 flex flex-col items-center text-center px-6 pb-6">
                                
                                {/* Date Circle - Điểm nhấn ngày tháng */}
                                <div className="bg-white p-1 rounded-full shadow-xl mb-4">
                                    <div className="w-20 h-20 rounded-full border border-rose-100 flex flex-col items-center justify-center bg-white">
                                        <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-0.5">Tháng {dateObj.month}</span>
                                        <span className="text-3xl font-serif font-black text-gray-800 leading-none">{dateObj.day}</span>
                                        <span className="text-[10px] text-rose-500 font-bold">{dateObj.year}</span>
                                    </div>
                                </div>

                                {/* Tên Cô Dâu Chú Rể - Font Great Vibes to, rõ */}
                                <div className="w-full mb-1">
                                    <h2 className="font-['Great_Vibes'] text-[42px] text-gray-800 leading-tight drop-shadow-sm" style={{ color: invitation.themeColor }}>
                                        {invitation.groomName} <span className="text-2xl text-gray-400 font-serif mx-1">&</span> {invitation.brideName}
                                    </h2>
                                </div>

                                {/* Thông tin tiệc - Font Be Vietnam Pro dễ đọc */}
                                <div className="space-y-1 mb-auto w-full">
                                    ...
                                    <p className="text-sm font-bold text-gray-800 uppercase tracking-wide px-4 line-clamp-1">
                                        {invitation.location || 'Địa điểm tổ chức'}
                                    </p>
                                    <p className="text-xs text-gray-500 italic px-8 line-clamp-2 leading-relaxed">
