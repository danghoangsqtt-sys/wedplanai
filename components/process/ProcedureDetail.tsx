...
import {
   ...
   Image as ImageIcon, X, ExternalLink, Search, Edit2, Save, Plus, Trash2, Link
} from 'lucide-react';
...
// Từ điển ánh xạ từ khóa sang hình ảnh minh họa (Unsplash source)
// Đây là dữ liệu mặc định (fallback)
...
const ProcedureDetail: React.FC<ProcedureDetailProps> = ({ step, region }) => {
   ...
   // Check if first step by checking ID against default first step for that region
   ...
   useEffect(() => {
      ...
      setActiveTab('INFO');
      setEditedStep(step); // Reset edit state form
      setIsEditing(false);
   }, [step]);

   // --- IMAGE HELPER: CUSTOM > DEFAULT ---
   const getImageForItem = (itemName: string): string | null => {
      // 1. Check Custom Images (Admin uploaded/linked) - Check both editedStep (preview) and step (saved)
   ...
   const handleImageUpdate = (itemName: string) => {
               ...
               <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl relative" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="relative aspect-video bg-gray-100 flex items-center justify-center overflow-hidden">
                     ...
                     <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 pt-12">
                        ...
                        <p className="text-white/80 text-xs">Hình ảnh mang tính chất minh họa</p>
                     </div>
                  </div>

                  <div className="p-4 bg-gray-50 flex flex-col gap-3">
                     <p className="text-sm text-gray-600 leading-snug">
                        Sính lễ và vật phẩm có thể khác nhau tùy theo vùng miền và điều kiện kinh tế.
                     </p>
                     <a
                        href={`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(viewingImage.name + " đám cưới việt nam")}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 w-full bg-white border border-gray-300 hover:border-rose-400 hover:text-rose-600 text-gray-700 py-2.5 rounded-xl font-bold transition-all shadow-sm text-sm"
                     >
                        <Search className="w-4 h-4" /> Tìm mẫu thực tế trên Google
                     </a>
                  </div>
               </div>
            </div>
            ...
            <div className="p-4 md:p-6 pb-2 md:pb-4 relative">
               ...
               {isEditing ? (
                  <div className="space-y-3 pr-20 animate-fadeIn">
                     <input
                        className="w-full text-xl md:text-2xl font-bold text-gray-800 border-b-2 border-rose-300 focus:border-rose-500 outline-none bg-transparent placeholder-gray-300"
                        value={editedStep.title}
                        onChange={e => setEditedStep({ ...editedStep, title: e.target.value })}
                        placeholder="Tiêu đề quy trình"
                     />
                     <textarea
                        className="w-full text-xs md:text-sm text-gray-600 border border-rose-200 focus:border-rose-400 rounded-lg p-2 outline-none h-20 bg-white shadow-inner resize-none"
                  ...
                  <>
                     <h2 className="text-xl md:text-2xl font-bold text-gray-800 flex flex-wrap items-center gap-2 pr-10">
                        ...
                        <span className="text-xs font-normal text-rose-600 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-full uppercase tracking-wide">
                           Miền {region === 'NORTH' ? 'Bắc' : region === 'CENTRAL' ? 'Trung' : 'Miền Nam'}
                        </span>
                     </h2>
               ...
               <div className="space-y-4 md:space-y-6 max-w-4xl mx-auto">
                     ...
                     <div className="bg-white p-4 md:p-5 rounded-xl border border-blue-100 shadow-[0_2px_8px_rgba(59,130,246,0.05)]">
                        ...
                        <div className="space-y-4 text-sm text-gray-600 leading-relaxed">
                           <div className="bg-blue-50/50 p-3 rounded-lg">
                              ...
                              {isEditing ? (
                                 <textarea
                                    className="w-full bg-white p-2 rounded border border-blue-200 outline-none focus:ring-2 focus:ring-blue-100 text-sm"
                                    value={editedStep.meaning}
                                    onChange={e => setEditedStep({ ...editedStep, meaning: e.target.value })}
                                    rows={4}
                                    placeholder="Nhập ý nghĩa nghi lễ..."
                                 />
                              ) : step.meaning}
                           </div>
                           <div>
                              ...
                              {isEditing ? (
                                 <textarea
                                    className="w-full bg-white p-2 rounded border border-gray-200 outline-none focus:ring-2 focus:ring-gray-100 text-sm"
                                    value={editedStep.participants}
                                    onChange={e => setEditedStep({ ...editedStep, participants: e.target.value })}
                                    rows={3}
                     ...
                     <div className="bg-white p-4 md:p-5 rounded-xl border border-rose-100 shadow-[0_2px_8px_rgba(244,63,94,0.05)]">
                        <div className="flex items-start gap-3 mb-4">
                           ...
                           <div>
                              ...
                              <p className="text-xs text-gray-400">Chuẩn bị chu đáo (Bấm để xem ảnh)</p>
                           </div>
                        </div>
                        <ul className="space-y-2">
                           {(isEditing ? editedStep.offerings : step.offerings).map((item, idx) => {
                              ...
                              return (
                                 <li
                                    key={idx}
                                    className={`flex items-start justify-between gap-2 text-sm text-gray-700 p-2 rounded transition-all group border border-transparent ${(!isEditing && (imgUrl || true)) ? 'hover:bg-rose-50 hover:border-rose-100 cursor-pointer' : ''} ${isEditing ? 'bg-gray-50' : ''}`}
                                 >
                                    <div className="flex-1 flex items-start gap-2" onClick={() => !isEditing && setViewingImage({ name: item, url: imgUrl })}>
                                       ...
                                       {isEditing ? (
                                          <div className="flex-1">
                                             <input
                                                className="w-full bg-white border border-gray-200 rounded px-2 py-1 text-sm focus:border-rose-300 outline-none"
                                                value={item}
                                                onChange={(e) => handleArrayChange('offerings', idx, e.target.value)}
                                                placeholder="Tên lễ vật..."
                                             />
                                          </div>
                                       ) : (
                                    ...
                                    {isEditing ? (
                                       <div className="flex gap-1 items-center">
                                          <button
                                             ...
                                             className={`p-1.5 rounded transition-colors ${imgUrl ? 'text-blue-500 bg-blue-50' : 'text-gray-300 hover:text-blue-500 hover:bg-gray-200'}`}
                                             title={imgUrl ? "Sửa link ảnh" : "Thêm link ảnh"}
                                          >
                                             <ImageIcon className="w-4 h-4" />
                                          </button>
                                          <button
                                             onClick={() => handleRemoveArrayItem('offerings', idx)}
                           ...
                           {isEditing && (
                              <button
                                 onClick={() => handleAddArrayItem('offerings')}
                                 className="w-full flex items-center justify-center gap-1.5 text-xs text-rose-500 font-bold mt-3 py-2 border border-dashed border-rose-300 rounded-lg hover:bg-rose-50 transition-colors"
                              >
                                 <Plus className="w-3.5 h-3.5" /> Thêm sính lễ
                              </button>
                           )}
                        </ul>
                     </div>
                     ...
                     <div className="space-y-4">
                        {(isEditing ? editedStep.tips : step.tips || []).map((t, i) => (
                           <div key={i} className="flex items-start gap-3 text-emerald-900 text-sm bg-white/60 p-3 rounded-lg border border-emerald-100/50 group">
                              ...
                              <div className="leading-relaxed flex-1">
                                 {isEditing ? (
                                    <textarea
                                       className="w-full bg-white p-2 rounded border border-emerald-200 outline-none focus:ring-1 focus:ring-emerald-300 text-sm"
                                       value={t}
                              ...
                              {isEditing && (
                                 <button
                                    onClick={() => handleRemoveArrayItem('tips', i)}
                        ...
                        {isEditing && (
                           <button
                              ...
                              className="w-full flex items-center justify-center gap-1.5 text-xs text-emerald-600 font-bold mt-2 py-2 border border-dashed border-emerald-300 rounded-lg hover:bg-emerald-100/50 transition-colors"
                           >
                              <Plus className="w-3.5 h-3.5" /> Thêm mẹo hay
                           </button>
                        )}
                     </div>
                  </div>
                  ...
                  <div className="bg-red-50 border-l-4 border-red-500 rounded-r-xl p-4 md:p-5 shadow-sm relative overflow-hidden">
                     ...
                     <ul className="space-y-2 relative z-10">
                        {(isEditing ? editedStep.taboos : step.taboos || []).map((t, i) => (
                           <li key={i} className="flex items-start gap-2 text-red-700 text-sm group">
                              ...
                              <div className="flex-1">
                                 {isEditing ? (
                                    <input
                                       className="w-full bg-white p-2 rounded border border-red-200 outline-none focus:ring-1 focus:ring-red-300 text-sm"
                                       value={t}
                                       onChange={e => handleArrayChange('taboos', i, e.target.value)}
                                    />
                                 ) : (
                              ...
                              {isEditing && (
                                 <button
                                    onClick={() => handleRemoveArrayItem('taboos', i)}
                                    className="p-1.5 text-red-300 hover:text-red-600 hover:bg-red-100 rounded transition-colors"
                                 >
                                    <Trash2 className="w-4 h-4" />
                                 </button>
                              )}
                        ...
                        {isEditing && (
                           <button
                              onClick={() => handleAddArrayItem('taboos')}
                              className="w-full flex items-center justify-center gap-1.5 text-xs text-red-600 font-bold mt-3 py-2 border border-dashed border-red-300 rounded-lg hover:bg-red-100/50 transition-colors"
                           >
                              <Plus className="w-3.5 h-3.5" /> Thêm điều kiêng kỵ
                           </button>
                        )}
                     </ul>
                  </div>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1">
                     ...
                     <div className="p-3 md:p-4 bg-white border-t border-gray-200 flex justify-end shadow-[0_-4px_12px_rgba(0,0,0,0.05)] flex-shrink-0 sticky bottom-0 z-10 lg:static">
                        <button
                           ...
                           disabled={justAdded}
                           className={`w-full sm:w-auto px-4 md:px-6 py-2.5 rounded-lg font-bold shadow-md flex items-center justify-center gap-2 transition-all transform active:scale-95 ${justAdded
                  ...
                  <div className="bg-gradient-to-br from-purple-50 via-white to-purple-50 p-4 md:p-6 rounded-xl border border-purple-100 flex-1 flex flex-col shadow-sm">
                     ...
                     {!aiContent ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-4 md:p-8 border-2 border-dashed border-purple-200 rounded-xl bg-white/60">
                           ...
                           {isRestricted && !isStepLocked && (
                              <span className="block text-center text-xs text-amber-600 font-bold mb-2">
                                 {isLimitReachedLocal ? "Đã hết lượt dùng thử" : `Dùng thử: Còn ${1 - guestUsage.speechCount} lượt`}
                              </span>
                           )}

                           {isStepLocked ? (
                              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-center max-w-md mx-auto">
                                 <Lock className="w-8 h-8 text-amber-500 mx-auto mb-2" />
                                 <p className="text-sm text-amber-800 font-bold mb-1">Tính năng giới hạn</p>
                                 <p className="text-xs text-amber-700">🔒 Tính năng này chỉ dành cho thành viên chính thức. Vui lòng liên hệ Admin.</p>
                              </div>
                           ) : (
                              <button
                                 onClick={handleGetSpeech}
                                 disabled={loadingAi || isLimitReachedLocal}
                                 className={`w-full sm:w-auto px-6 md:px-8 py-3 rounded-full font-bold shadow-lg flex items-center justify-center gap-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed text-sm md:text-base ${isLimitReachedLocal ? 'bg-gray-400 text-white shadow-none cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white shadow-purple-200 hover:-translate-y-1'}`}
                              >
                                 {isLimitReachedLocal ? <Lock className="w-5 h-5" /> : (loadingAi ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />)}
                                 {isLimitReachedLocal ? "Hết lượt miễn phí (Liên hệ Admin)" : (loadingAi ? "Đang soạn thảo..." : "Viết bài phát biểu mẫu")}
                              </button>
                           )}
                        </div>
                     ) : (
                        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-md border border-purple-100 overflow-hidden relative min-h-[400px]">
                           ...
                           <div className="flex-1 p-4 md:p-6 overflow-y-auto bg-white relative">
                              <textarea
                                 className="w-full h-full resize-none focus:outline-none font-serif text-gray-800 leading-relaxed text-sm md:text-base bg-transparent min-h-[300px]"
                                 value={aiContent}
                                 readOnly
                              />
                           </div>
                           <div className="p-3 bg-gray-50 border-t border-gray-100 flex justify-center flex-shrink-0">
                              <button
                                 onClick={handleGetSpeech}
                                 className={`text-xs font-medium underline transition-colors ${isLimitReachedLocal ? 'text-gray-400' : 'text-gray-500 hover:text-purple-600'}`}
                              >
                                 {isLimitReachedLocal ? "Đã hết lượt soạn lại" : "Thử soạn lại nội dung khác"}
                              </button>
                           </div>
                        </div>
                     )}
