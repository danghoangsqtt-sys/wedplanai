...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onClose }) => {
  ...
  const handleGoogleLogin = () => {
    setIsLoading(true);
    setError('');
    // Redirect-based OAuth — browser navigates to Google, then back to app.
    // App.tsx initSession() detects the new session on return.
      ...
      <div className="p-8 text-center">
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold py-3 px-4 rounded-xl transition-all shadow-sm active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? (
          ...
          {error && (
            <div className="flex items-center gap-2 text-red-600 text-xs bg-red-50 p-3 rounded-lg border border-red-100 text-left">
