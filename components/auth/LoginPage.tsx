...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onClose }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 1. Kiểm tra trên Cloud trước (quan trọng để không ghi đè Admin thành User thường)
      let targetUser: UserProfile | null = await getUserPublicProfile(fbUser.uid);
      const isAdminEmail = fbUser.email === 'danghoang.sqtt@gmail.com';

      if (targetUser) {
          // User đã tồn tại trên hệ thống Cloud -> Cập nhật thông tin mới nhất từ Google
          targetUser = {
              ...targetUser,
              email: fbUser.email,
              displayName: fbUser.displayName || targetUser.displayName,
              photoURL: fbUser.photoURL || targetUser.photoURL,
              // Admin hardcode luôn được bảo vệ quyền
              role: isAdminEmail ? 'ADMIN' : targetUser.role,
              isActive: isAdminEmail ? true : targetUser.isActive,
              allowCustomApiKey: isAdminEmail ? true : targetUser.allowCustomApiKey
          };
          // Đồng bộ lại thông tin mới lên Cloud
          await syncUserProfile(targetUser);
      } else {
          // User hoàn toàn mới -> Tạo profile
          targetUser = {
            uid: fbUser.uid,
            email: fbUser.email,
