...
const getElementConfig = (key: ElementKey | undefined) => {
  switch (key) {
    ...
    case 'MOC': return { label: 'Mộc', color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', icon: Wind }; // Wind as proxy for Wood/Nature
    ...
    <div className="flex flex-col h-full bg-[#FCFAF5] overflow-y-auto font-sans relative">
      ...
      <div className="flex justify-center bg-white shadow-sm sticky top-0 z-20 border-b border-amber-100">
        <button 
           ...
           className={`px-6 py-3 text-sm font-bold border-b-2 transition-all ${activeTab === 'PROFILE' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
        >
          Hồ Sơ
        </button>
        <button 
           ...
           className={`px-6 py-3 text-sm font-bold border-b-2 transition-all ${activeTab === 'HARMONY' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 disabled:opacity-50'}`}
        >
          Luận Tuổi
        </button>
        <button 
           ...
           className={`px-6 py-3 text-sm font-bold border-b-2 transition-all ${activeTab === 'DATES' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 disabled:opacity-50'}`}
        >
          Ngày Lành
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto w-full p-4 md:p-8 flex-1 z-10 relative">
          ...
          <div className="space-y-6 animate-fadeIn">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 relative overflow-hidden group hover:shadow-md transition-all">
                   ...
                   <div className="relative z-10">
                      <div className="flex items-center gap-3 mb-6">
                         <div className="bg-blue-600 text-white p-2.5 rounded-full shadow-lg shadow-blue-200">
                            <Moon className="w-5 h-5" />
                         </div>
                         <div>
                            <h3 className="font-bold text-gray-800 text-lg">Chú Rể</h3>
                            <p className="text-xs text-blue-500 font-bold uppercase tracking-wider">Dương Nam</p>
                         </div>
                      </div>
                      <div className="space-y-4">
                         <div>
                            ...
                            <input 
                              className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-blue-500 outline-none font-medium text-gray-800"
                         ...
                         <div className="grid grid-cols-2 gap-3">
                            <div>
                               <label className="text-xs font-bold text-gray-400 uppercase">Ngày sinh (DL)</label>
                               <input type="date" className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-blue-500 outline-none" 
                                  value={formData.groomDob} onChange={e => handleInputChange('groomDob', e.target.value)}/>
                            </div>
                            <div>
                               <label className="text-xs font-bold text-gray-400 uppercase">Giờ sinh</label>
                               <input type="time" className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-blue-500 outline-none" 
                                  value={formData.groomTime} onChange={e => handleInputChange('groomTime', e.target.value)}/>
                            </div>
                         </div>
                      </div>
                   </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-pink-100 relative overflow-hidden group hover:shadow-md transition-all">
                   ...
                   <div className="relative z-10">
                      <div className="flex items-center gap-3 mb-6">
                         <div className="bg-pink-600 text-white p-2.5 rounded-full shadow-lg shadow-pink-200">
                            <Sun className="w-5 h-5" />
                         </div>
                         <div>
                            <h3 className="font-bold text-gray-800 text-lg">Cô Dâu</h3>
                            <p className="text-xs text-pink-500 font-bold uppercase tracking-wider">Âm Nữ</p>
                         </div>
                      </div>
                      <div className="space-y-4">
                         <div>
                            <label className="text-xs font-bold text-gray-400 uppercase">Họ và tên</label>
                            <input 
                              className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-pink-500 outline-none font-medium text-gray-800"
                         ...
                         <div className="grid grid-cols-2 gap-3">
                            <div>
                               <label className="text-xs font-bold text-gray-400 uppercase">Ngày sinh (DL)</label>
                               <input type="date" className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-pink-500 outline-none" 
                                  value={formData.brideDob} onChange={e => handleInputChange('brideDob', e.target.value)}/>
                            </div>
                            <div>
                               <label className="text-xs font-bold text-gray-400 uppercase">Giờ sinh</label>
                               <input type="time" className="w-full mt-1 p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-pink-500 outline-none" 
                                  value={formData.brideTime} onChange={e => handleInputChange('brideTime', e.target.value)}/>
                            </div>
                         </div>
                      </div>
                   </div>
             ...
             <div className="flex justify-center pt-6">
                <button 
                  ...
                  disabled={loading}
                  className="bg-red-700 hover:bg-red-800 text-yellow-50 px-10 py-4 rounded-full font-bold shadow-lg shadow-red-200 flex items-center gap-3 transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed group relative overflow-hidden"
                >
                   ...
                   {loading ? (
                     ...
                     <>
                        ...
                        <span className="tracking-wide">Luận Giải Duyên Phận</span>
                     </>
                   )}
                </button>
             </div>
        ...
        {activeTab === 'HARMONY' && fengShuiResults.harmony && (
          <div className="space-y-6 animate-fadeIn pb-10">
             ...
             {/* 1. Overview Card with Element Visualizer */}
             <div className="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden">
                <div className="bg-amber-50/50 p-4 border-b border-amber-100 flex justify-between items-center">
                   <h3 className="font-bold text-amber-900 flex items-center gap-2">
                      <Star className="w-5 h-5 text-amber-500 fill-current" /> Tổng Quan Ngũ Hành
                   </h3>
                   <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                     fengShuiResults.harmony.conflictStatus === 'SINH' ? 'bg-green-100 text-green-700 border-green-200' :
                     fengShuiResults.harmony.conflictStatus === 'KHAC' ? 'bg-red-100 text-red-700 border-red-200' :
                     'bg-gray-100 text-gray-700 border-gray-200'
                   }`}>
                      {fengShuiResults.harmony.conflictStatus === 'SINH' ? 'TƯƠNG SINH (TỐT)' :
                       fengShuiResults.harmony.conflictStatus === 'KHAC' ? 'TƯƠNG KHẮC (XẤU)' : 'BÌNH HÒA'}
                   </span>
                </div>
                ...
                <div className="p-6 md:p-8 flex flex-col md:flex-row items-center justify-around gap-8">
                   ...
                   <div className="flex flex-col items-center justify-center">
                      <div className={`w-32 h-1 rounded-full ${
                          fengShuiResults.harmony.conflictStatus === 'SINH' ? 'bg-green-200' : 
                          fengShuiResults.harmony.conflictStatus === 'KHAC' ? 'bg-red-200' : 'bg-gray-200'
                      } relative`}>
                         <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-2 rounded-full border shadow-sm ${
                            fengShuiResults.harmony.conflictStatus === 'SINH' ? 'border-green-200 text-green-600' : 
                            fengShuiResults.harmony.conflictStatus === 'KHAC' ? 'border-red-200 text-red-600' : 'border-gray-200 text-gray-400'
                         }`}>
                ...
                <div className="bg-gray-50 p-4 border-t border-gray-100 flex items-center justify-center gap-4">
                   ...
                   <div className={`text-3xl font-black ${getScoreColor(fengShuiResults.harmony.score).split(' ')[0]}`}>
                      {fengShuiResults.harmony.score}/100
                   </div>
             ...
             <div className="bg-white rounded-2xl shadow-sm border border-amber-100 p-6 md:p-8 relative">
                ...
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-serif font-bold text-2xl text-red-900 flex items-center gap-2">
                       <Sparkles className="w-5 h-5 text-yellow-500" /> Lời Phán Của Thầy
                    </h3>
                    <button 
             ...
             <div className="bg-gradient-to-r from-red-50 to-amber-50 rounded-2xl border border-red-100 p-6 flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="flex-1 text-center md:text-left">
                   <h4 className="font-bold text-gray-800 text-lg">Đã xem tuổi, hãy chọn ngày lành!</h4>
                   <p className="text-sm text-gray-600">Thầy sẽ tìm cho hai bạn 5 ngày đại cát để tổ chức hôn lễ.</p>
                </div>
                <div className="flex flex-col w-full md:w-auto gap-2">
                   <input 
                      className="p-3 rounded-lg border border-red-200 outline-none focus:border-red-500 text-sm w-full md:w-64"
                   ...
                   <button 
                      ...
                      disabled={loading}
                      className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                   >
                 ...
                 <div key={idx} className="bg-white rounded-xl border border-amber-100 p-0 shadow-sm hover:shadow-lg transition-all relative overflow-hidden group flex flex-col">
                    ...
                    <div className="p-5 flex-1">
                       ...
                       <div className="space-y-3 mb-4">
                          <div className="flex gap-3">
                             ...
                             <p className="text-sm text-gray-600 leading-snug">{date.reason}</p>
                          </div>
                          <div className="flex gap-3">
                             <Clock className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
                             <p className="text-sm text-gray-600"><span className="font-bold text-gray-700">Giờ đẹp:</span> {date.timeSlots}</p>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 border-t border-gray-100 mt-auto">
                       <button 
                          ...
                          className="w-full py-2.5 bg-white border border-gray-300 hover:border-red-500 hover:text-red-600 text-gray-600 font-bold rounded-lg transition-colors flex items-center justify-center gap-2 text-sm shadow-sm"
                       >
