...
// 2. Kiểm tra tên hợp lệ (Chỉ chữ cái, không số, không ký tự lạ, không lặp nhảm)
const isValidName = (name: string): boolean => {
   const trimmed = name.trim();
   if (trimmed.length < 2 || trimmed.length > 50) return false;

   // Regex: Chấp nhận chữ cái Tiếng Việt và khoảng trắng. Không chấp nhận số hoặc ký tự đặc biệt.
   ...
   // Kiểm tra lặp ký tự 3 lần liên tiếp (chống spam kiểu 'kkk', 'aaa')
...
const FengShuiConsultant: React.FC = () => {
   ...
   const handleAnalyze = async () => {
      ...
      // 2. Kiểm tra Tên (Logic mới)
      if (!isValidName(formData.groomName)) {
         alert("Tên Chú Rể không hợp lệ.\nVui lòng nhập tên thật (Ví dụ: Nguyễn Văn An), không chứa số, ký tự đặc biệt hoặc viết tắt vô nghĩa.");
         return;
      }
      if (!isValidName(formData.brideName)) {
         alert("Tên Cô Dâu không hợp lệ.\nVui lòng nhập tên thật (Ví dụ: Trần Thị B), không chứa số, ký tự đặc biệt hoặc viết tắt vô nghĩa.");
         return;
      }

      // 3. Kiểm tra Tuổi (Logic mới)
      const groomAge = calculateAge(formData.groomDob);
      const brideAge = calculateAge(formData.brideDob);
      const currentYear = new Date().getFullYear();

      // Validate năm sinh tương lai
      ...
      // Validate độ tuổi kết hôn (Luật VN: Nam >= 20, Nữ >= 18)
      if (groomAge < 20) {
         alert(`Chú rể mới ${groomAge} tuổi.\nTheo luật Hôn nhân & Gia đình Việt Nam, nam phải từ đủ 20 tuổi trở lên mới đủ điều kiện kết hôn.`);
         return;
      }
      if (brideAge < 18) {
         alert(`Cô dâu mới ${brideAge} tuổi.\nTheo luật Hôn nhân & Gia đình Việt Nam, nữ phải từ đủ 18 tuổi trở lên mới đủ điều kiện kết hôn.`);
         return;
      }

      // Validate tuổi quá cao (tránh nhập sai thế kỷ)
      if (groomAge > 100 || brideAge > 100) {
         if (!confirm(`Hệ thống phát hiện tuổi khá cao (Chồng: ${groomAge}, Vợ: ${brideAge}).\nBạn có chắc chắn ngày sinh đã nhập đúng không?`)) {
            return;
         }
      }
   ...
   return (
      <div className="flex flex-col h-full bg-[#FCFAF5] overflow-y-auto font-sans relative">
         ...
         <div className="flex-shrink-0 z-20 bg-white shadow-sm">
            {/* Header Banner */}
            <div className="bg-gradient-to-r from-red-900 via-red-800 to-red-900 text-amber-50 p-4 md:p-6 flex flex-col items-center justify-center relative overflow-hidden">
               ...
               <Sparkles className="w-6 h-6 md:w-8 md:h-8 mb-2 animate-pulse text-yellow-400 relative z-10" />
               <h1 className="text-xl md:text-2xl font-bold uppercase tracking-[0.2em] font-serif text-yellow-50 relative z-10 text-center">Phong Thủy AI</h1>
               <p className="text-[10px] md:text-xs text-yellow-200/80 font-serif italic mt-1 relative z-10">Hòa Hợp Âm Dương • Vạn Sự Như Ý</p>
            </div>

            {/* Navigation Tabs (Sticky) */}
            <div className="flex justify-center border-b border-amber-100 overflow-x-auto no-scrollbar">
               <button onClick={() => setActiveTab('PROFILE')} className={`px-4 md:px-6 py-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap ${activeTab === 'PROFILE' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Hồ Sơ</button>
               <button onClick={() => setActiveTab('HARMONY')} disabled={!fengShuiResults.harmony} className={`px-4 md:px-6 py-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap ${activeTab === 'HARMONY' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 disabled:opacity-50'}`}>Luận Tuổi</button>
               <button onClick={() => setActiveTab('COMBINED')} disabled={!fengShuiResults.harmony?.combinedAnalysis} className={`px-4 md:px-6 py-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap ${activeTab === 'COMBINED' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-500 disabled:opacity-50'}`}>Đa Chiều (Mới)</button>
               <button onClick={() => setActiveTab('DATES')} disabled={!fengShuiResults.dates.length} className={`px-4 md:px-6 py-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap ${activeTab === 'DATES' ? 'border-red-600 text-red-700 bg-red-50' : 'border-transparent text-gray-500 disabled:opacity-50'}`}>Ngày Lành</button>
            </div>
         </div>

         {/* 2. PHẦN NỘI DUNG CUỘN (Scrollable Content) */}
         <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10">
            <div className="max-w-4xl mx-auto">
               ...
               {activeTab === 'COMBINED' && fengShuiResults.harmony?.combinedAnalysis && (
                  <div className="space-y-6 animate-fadeIn pb-10">
                     <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl shadow-lg text-white p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                              <div className="flex items-center gap-2 mb-2 font-bold text-purple-100 uppercase text-sm tracking-wider border-b border-white/20 pb-2"><Moon className="w-4 h-4" /> CHỒNG</div>
                              <div className="flex justify-between items-center mb-2">
                                 <span className="text-sm opacity-80">Hoàng Đạo:</span>
                                 <span className="font-bold text-lg">{fengShuiResults.harmony.combinedAnalysis.groomZodiac}</span>
                              </div>
                              <div className="flex justify-between items-center">
                                 <span className="text-sm opacity-80">Số Chủ Đạo:</span>
                                 <span className="font-bold text-2xl bg-white text-purple-600 w-8 h-8 flex items-center justify-center rounded-full shadow-lg">
                                    {fengShuiResults.harmony.combinedAnalysis.groomLifePath}
                                 </span>
                              </div>
                           </div>
                           <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
                              <div className="flex items-center gap-2 mb-2 font-bold text-purple-100 uppercase text-sm tracking-wider border-b border-white/20 pb-2"><Sun className="w-4 h-4" /> VỢ</div>
                              <div className="flex justify-between items-center mb-2">
                                 <span className="text-sm opacity-80">Hoàng Đạo:</span>
                                 <span className="font-bold text-lg">{fengShuiResults.harmony.combinedAnalysis.brideZodiac}</span>
                              </div>
                              <div className="flex justify-between items-center">
                                 <span className="text-sm opacity-80">Số Chủ Đạo:</span>
                                 <span className="font-bold text-2xl bg-white text-indigo-700 w-8 h-8 flex items-center justify-center rounded-full shadow-lg">
                     ...
                     <div className="bg-white rounded-2xl shadow-sm border border-purple-100 p-6 md:p-8">
                        <h3 className="font-bold text-xl text-purple-900 mb-4 flex items-center gap-2">
                           <Hash className="w-5 h-5" /> Tổng Hợp Đa Chiều
                        </h3>
